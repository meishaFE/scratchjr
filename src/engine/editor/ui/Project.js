...
export default class Project {
  ...
  static load(data, loadDone) {
    mediaCountBase = 1;
    files = data.files;

    if (BlockSpecs.loadCount > 0) {
      setTimeout(function() {
        Project.delayLoad(data, loadDone);
      }, 32);
    } else {
      Project.startLoad(data, loadDone);
    }
  }

  static delayLoad(data, loadDone) {
   ...
   * 新增加载空项目
   */
  static async startLoad(data, loadDone) {
    UI.layout();

    Project.dataRecieved(data, loadDone);
  }
   ...
   */
  static dataRecieved(data, loadDone) {
    metadata = data;
    mediaCount = -1;
    if (metadata.json) {
      Project.loadData(data, doneProjectLoad);
    } else {
      mediaCount = 0;
  ...
  static recreate(data) {
    const dataJson = data.json.json;
    mediaCount = 0;
    ScratchJr.stage.pages = [];
    var pages = dataJson.pages;
    pageid = dataJson.currentPage;
    for (var i = 0; i < pages.length; i++) {
      Project.recreatePage(pages[i], dataJson[pages[i]]);
    }
    mediaCountBase = mediaCount;
  }
   ...
   * @returns {object} 项目数据
   */
  static saveLocalPromise(id) {
    return new Promise((resolve, reject) => {
      if (saving) {
        Alert.open(frame, gn('flip'), 'Waiting', '#28A5DA');
        Project.waitUntilSaved(id, resolve);
      } else {
        Alert.open(frame, gn('flip'), 'Saving', '#28A5DA');
        Project.save(id, resolve);
      }
    });
  }
  ...
  static save(id, whenDone) {
    ...
    function saveDone() {
      saving = false;
      if (whenDone) {
        whenDone(JSON.parse(JSON.stringify(metadata)));
      }
    }
  }
