...
export default class Stage {
  ...
  setPage(page, isOn) {
    ...
    var sc = ScratchJr.getSprite()
      ? gn(ScratchJr.stage.currentPage.currentSpriteName + '_scripts')
      : undefined;
    if (sc) {
      sc.owner.deactivate();
    }
  ...
  mouseDown(e) {
    ...
    let pt = { x: 0, y: 0 };
    let hitobj = e.target.owner ? e.target.owner : e.target.parentNode.owner;

    if (hitobj && hitobj.type === 'sprite') {
