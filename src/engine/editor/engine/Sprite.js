...
export default class Sprite {
  ...
  getAsset(whenDone) {
    var md5 = this.md5;
    var spr = this;
    var url = MediaLib.keys[md5] ? MediaLib.path + md5 : md5;
    md5 = MediaLib.keys[md5] ? MediaLib.path + md5 : md5;

    if (md5.indexOf('/') > -1) {
      IO.requestFromServer(md5, doNext);
    } else {
      let b64 = Project.files[md5].split(',')[1];
      let str = atob(b64);
      doNext(str);
    }
  ...
  setTextBox() {
    ...
    if (isAndroid) {
      ...
      //   (dy + ti.parentNode.parentNode.getBoundingClientRect().height * 1.7) *
  ...
  activate() {
    var list = fitInRect(
      this.w,
      this.h,
      ScriptsPane.watermark.offsetWidth,
      ScriptsPane.watermark.offsetHeight
    );
  ...
  stopShaking() {
    ...
    if (isAndroid) {
      ...
      var mtx =
        'translate3d(' + (this.xcoor - this.cx) + 'px,' + (this.ycoor - this.cy) + 'px, 0px)';
      if (this.img) {
        mtx += ' rotate(' + this.angle + 'deg)';
        if (this.flip) {
          mtx += 'scale(' + -this.scale + ', ' + this.scale + ')';
