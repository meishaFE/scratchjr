...
export default class Page {
  ...
  setBackground(name, fcn) {
    ...
    var url = MediaLib.keys[name] ? MediaLib.path + name : name;

    if (MediaLib.keys[name]) {
      this.setBackgroundImage(url, fcn);
    } else if (Project.files[name]) {
      // 加载base64
      this.setBackgroundImage(Project.files[name], fcn);
    }
  }
  ...
  encodePage() {
    ...
    this.currentSpriteName = !this.currentSpriteName
      ? undefined
      : gn(this.currentSpriteName).owner.type == 'sprite'
