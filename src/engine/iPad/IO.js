...
export default class IO {
  ...
  static getAsset(md5, fcn) {
    ...
    function nextStep(dataurl) {
      // iOS 7 requires to read the internal base64 images before returning contents
  ...
  static createProject(obj, fcn) {
    let json = {};
    let keylist = ['name', 'version', 'deleted', 'ctime', 'mtime', 'isGift', 'userid', 'projectid'];
    // let keylist = ['name', 'version', 'deleted', 'mtime', 'isGift', 'userid', 'projectid'];
    let values = '?,?,?,?,?,?,?,?';
    let ctime = obj.ctime ? obj.ctime : new Date().getTime().toString();
    let mtime = obj.mtime ? obj.mtime : new Date().getTime().toString();
    let isGift = obj.isgift ? obj.isgift : '0';
    let userid = obj.userid ? obj.userid : '';
    let projectid = obj.projectid ? obj.projectid : '';
    json.values = [obj.name, obj.version, 'NO', ctime, mtime, isGift, userid, projectid];
    // json.values = [obj.name, obj.version, 'NO', mtime, isGift, userid, projectid];

    if (obj.json) addValue('json', JSON.stringify(obj.json));
    if (obj.thumbnail) addValue('thumbnail', JSON.stringify(obj.thumbnail));
        ...
        if (md5 && typeof md5 !== 'undefined') {
          if (md5.indexOf('samples/') < 0) {
            ...
            if (collectLibraryAssets) {
              ...
              if (
                md5 &&
                typeof md5 !== 'undefined' &&
                !MediaLib.keys[md5] &&
                MediaLib.sounds.indexOf(md5) < 0
              ) {
  ...
  static uniqueProjectName(jsonData, callback, useOne) {
    ...
    IO.query(iOS.database, json, function(existingProjects) {
      ...
      if (
        newNumber != null &&
        (!giftProjectNameParts.number || newNumber > giftProjectNameParts.number)
      ) {
  ...
  static loadProjectFromSjr(b64data, action, projectLocal, projectServer) {
    ...
    receivedZip.filter((relativePath, file) => {
      ...
      if (fullName === 'data.json') {
        ...
        if (projectVersion > currentVersion) {
          throw new Error(
            'Project created in a new version of ScratchJr. Please upgrade ScratchJr.'
          );
        }

        if (projectLocal) {
          // 同步过来的作品id和本地的作品id不一样
          jsonData.id = projectLocal.id;
        }

        if (projectServer) {
          jsonData.projectid = projectServer.id;
          jsonData.userid = projectServer.userId;
        }

        if (action === 'create') {
          IO.createProject(jsonData, () => {});
        } else {
          IO.saveProject(jsonData, () => {});
        }
        ...
        iOS.setmedianame(b2data, name, ext, function() {
          ...
          function gotSVGImages() {
            ...
            iOS.setmedia(thumbnailPngBase64, 'png', function(thumbnailMD5) {
              ...
              json.cond =
                'ext = ? AND md5 = ? AND altmd5 = ? AND name = ? ' +
              ...
              IO.query('usershapes', json, function(results) {
                ...
                if (results.length === 0) {
                  ...
                  json.stmt =
                    'insert into usershapes (' + keylist.toString() + ') values (' + values + ')';
            ...
            iOS.setmedia(thumbnailPngBase64, 'png', function(thumbnailMD5) {
              ...
              IO.query('userbkgs', json, function(results) {
                ...
                if (results.length === 0) {
                  ...
                  json.stmt =
                    'insert into userbkgs (' + keylist.toString() + ') values (' + values + ')';
                  iOS.stmt(json, () => {});
                }
              });
            });
